        ...
        case APP_STATE_SCHEDULE_WRITE:
            ...
            if (appData.isReadComplete) {
                ...
                if (go==1 & i<200){
                    ...
                    accelZ=gyroShorts[6];
                    for (k=9; k>1; k--){
                        accelZbuf[k]=accelZbuf[k-1];
                    }
                    accelZbuf[0]=(float) accelZ;
                    
                    for (l=0; l<10; l++){
                        firAvg+=accelZbuf[l]*firWeights[l];
                        mafAvg+=accelZbuf[l]*1/10;
                    }
                    
                    iirAvg=iirA*iirPrev+iirB*accelZ;
                    
                    len = sprintf(dataOut, "%d\t%d\t%10.4f\t%10.4f\t%10.4f\r\n", i,accelZ,mafAvg,iirAvg,firAvg);
                    i++;
                    USB_DEVICE_CDC_Write(USB_DEVICE_CDC_INDEX_0,
                        &appData.writeTransferHandle, dataOut, len,
                        USB_DEVICE_CDC_TRANSFER_FLAGS_DATA_COMPLETE);
